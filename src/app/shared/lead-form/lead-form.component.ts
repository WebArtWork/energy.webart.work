...
export type LeadFormKind = 'consumer' | 'installer' | 'producer' | 'supplier';

const KIND_LABELS: Record<LeadFormKind, string> = {
	consumer: 'Заявка споживача',
	installer: 'Заявка інсталятора',
	producer: 'Заявка виробника енергії',
	supplier: 'Заявка постачальника',
};

/** Inline lead-capture form used on the /for-* landing pages. Sends to the Energy Telegram contact channel. */
@Component({
...
export class LeadFormComponent {
	...
	protected async submit(form: NgForm): Promise<void> {
		...
		const lines = [
			`Energy lead: ${KIND_LABELS[this.kind()]}`,
			`Ім'я: ${this.name()}`,
